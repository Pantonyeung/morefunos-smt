...
const navItems=[
  ['order','點單','<path d="M5 5h14v14H5z"/><path d="M8 9h8M8 13h5"/>'],
  ['checkout','收銀','<path d="M5 7h14v10H5z"/><path d="M8 11h8M9 15h3"/>'],
...
export function renderGlobalStatusBar({terminalId='SMT-T2S',operationLabel='接單中',operationTone='online',lastOrder='—',context='',rightActions=''}={}){
...
export function installShellNavigation(root=document){
  if(root.__morefunShellNavInstalled)return;
  root.__morefunShellNavInstalled=true;
  root.addEventListener('click',event=>{
    const button=event.target.closest('[data-action="shell-navigate"]');
    if(!button)return;
    const route=button.dataset.route||'order';
    const target=route==='order'?'../order/index.html':`../${route}/index.html`;
