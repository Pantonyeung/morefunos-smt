...
const drinkNeedles=['.drink-choice-card','.drink-choice-img','.drink-choice-count','.drink-card--drawer','.drink-card--detail','.drink-card--completion'];
...
function isDrinkOwnedSelector(selector){
  const normalized=selector.replace(/\s+/g,' ').trim();
  if(!normalized)return false;
  return drinkNeedles.some(needle=>normalized.includes(needle));
}
...
export function cleanupDrinkCardLegacy(css){
  let cursor=0;
  let output='';
  ...
  while(cursor<css.length){
    ...
    const kept=selectors.filter(selector=>!isDrinkOwnedSelector(selector));
...
function assertOwnershipBoundary(css){
  for(const needle of drinkNeedles){
    if(css.includes(needle))throw new Error(`Drink Card legacy selector remains: ${needle}`);
  }
  const mustRemain=['.product-card','.specified-link-card','.pairing-body','.catalog','.detail-drinks','.completion-drinks'];
  for(const needle of mustRemain){
    if(!css.includes(needle))throw new Error(`Non-Drink authority was removed unexpectedly: ${needle}`);
  }
}

function assertSafeMigration(result){
  if(result.removedSelectors<8)throw new Error(`Expected at least 8 Drink Card selectors, removed ${result.removedSelectors}`);
  if(result.removedSelectors>60)throw new Error(`Refusing unexpectedly broad Drink Card cleanup: ${result.removedSelectors} selectors`);
...
function main(){
  ...
  const result=cleanupDrinkCardLegacy(source);

  if(source===result.css){
    assertOwnershipBoundary(source);
    console.log('V2_DRINK_CARD_ALREADY_CLEARED');
    return;
  }

  assertSafeMigration(result);
  console.log(`V2_DRINK_CARD_CLEANUP selectors=${result.removedSelectors} rules=${result.removedRules} rewritten=${result.rewrittenRules}`);
  if(check){
    process.exitCode=2;
    console.log('V2_DRINK_CARD_CLEANUP_REQUIRED');
    return;
  }
