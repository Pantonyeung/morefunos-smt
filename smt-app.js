...
function freshState(){return {
  ...
  pendingGroupOpen:{'磨飯 App':true,'電話／WhatsApp':true},quickModeEnabled:true,
...
function orderPage(){const visible=DATA.products.filter(product=>state.category==='全部'||(state.category==='人氣推薦'&&product.category==='人氣推薦')||product.category===state.category);return `<div class="app-shell">${topbar()}<main class="workspace"><section class="order-grid"><aside class="cart-panel panel"><header><h2>購物車（${cartCount()}）</h2><div><button data-action="save-draft">暫存</button><button data-action="clear-cart" ${state.cart.length?'':'disabled'}>清空</button></div></header><div class="cart-list">${cartRows()}</div>${pendingReceipt()}<footer><button class="secondary" data-action="open-completion">整理待補</button><button class="primary" data-action="go-checkout" ${state.cart.length?'':'disabled'}>${pendingSummary().total?'先完成必選項目':`結帳 ${money(cartTotal())}`}</button></footer></aside><section class="catalog-panel panel"><div class="categories">${DATA.categories.map(category=>`<button data-action="category" data-value="${category}" class="${state.category===category?'active':''}">${category}</button>`).join('')}</div><div class="products products-${state.settings.catalog.defaultTemplate}">${visible.map(productCard).join('')}</div>${quickStrip()}</section>${cardLayer()}</section></main>${bottomNav()}</div><div id="overlay-root"></div><div id="toast" class="toast"></div>`;}
...
function handleAction(event){const element=event.target.closest('[data-action]');if(!element)return;const action=element.dataset.action;
  ...
  else if(action==='open-product'){if(state.quickModeEnabled)quickAddProduct(element.dataset.id);else openProduct(element.dataset.id);}
  ...
  else if(action==='close-card'){if(state.card==='edit'&&state.modal?.dirty)return openConfirm('尚未套用修改','離開會失去今次修改。',()=>{state.card=null;state.modal=null;render();});state.card=null;if(state.modal?.lineId)state.modal=null;render();}
  ...
  else if(action==='toggle-quick-mode'){state.quickModeEnabled=!state.quickModeEnabled;render();}
  ...
  else if(action==='cancel-confirm'){const previousModal=state.modal?.previousModal;state.modal=previousModal||null;render();}
