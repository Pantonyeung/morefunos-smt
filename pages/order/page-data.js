...
export function addProductToCart(cart,product,{quickMode=false,options={},drink=null}={}){if(!product)return cart;const decision=getProductAddDecision(product,quickMode);if(decision==='configure-before-add'&&!drink&&(product.requiredGroups||[]).some(group=>group==='drink'||!options[group]))return cart;return [...cart,createCartLine(product,{options,drink})]}
export function getLineMissingRequirements(line){const missing=[];for(const group of line.requiredGroups||[]){if(group==='drink'){const count=Math.max(0,(line.drinkSlots||0)-(line.drinkAssignments||[]).length);if(count)missing.push({lineId:line.lineId,group:'drink',label:'飲品',count})}else if(!line.options?.[group]){const label=group==='rice'?'飯底':group==='sauce'?'醬汁':group;missing.push({lineId:line.lineId,group,label,count:1})}}return missing}
export function getPendingState(cart){const requiredMissing=cart.flatMap(getLineMissingRequirements);const requiredMissingCount=requiredMissing.reduce((sum,item)=>sum+item.count,0);return {requiredMissing,requiredMissingCount,combinable:[],canCheckout:requiredMissingCount===0}}
...
export function addQuickDrink(cart,selection){if(getMissingDrinkSlots(cart)<=0)return cart;const index=cart.findIndex(line=>(line.drinkSlots||0)>(line.drinkAssignments||[]).length);if(index<0)return cart;return cart.map((line,i)=>i===index?{...line,drinkAssignments:[...(line.drinkAssignments||[]),clone(selection)]}:line)}
...
export function describeLine(line){const parts=[];if(line.options?.rice)parts.push(line.options.rice);if(line.options?.sauce)parts.push(line.options.sauce);if(line.options?.adjustment)parts.push(line.options.adjustment);const groups={};for(const item of line.drinkAssignments||[]){const key=[item.name,item.sweetness||'正常甜',item.ice||'正常冰'].join('|');groups[key]=(groups[key]||0)+1}for(const [key,qty] of Object.entries(groups)){const [name,sweetness,ice]=key.split('|');const optionText=[sweetness,ice].filter(value=>!value.startsWith('正常')).join('');parts.push(`${name}${optionText?` · ${optionText}`:''}${qty>1?` ×${qty}`:''}`)}const missing=Math.max(0,(line.drinkSlots||0)-(line.drinkAssignments||[]).length);if(missing)parts.push(`尚欠飲品 ${missing} 份`);return parts.length?parts.join(' · '):(line.detail||line.code||'標準')}
