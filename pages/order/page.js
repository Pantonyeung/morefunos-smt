...
function quickDrinks(){
  ...
  return '<section class="quick-drawer '+(state.quickDrawerOpen?'open':'')+'"><button class="quick-drawer-handle" data-action="toggle-quick-drawer"><span>快捷飲品</span><em>待補 '+missing+'</em><b>'+(state.quickDrawerOpen?'⌄':'⌃')+'</b></button>'+(state.quickDrawerOpen?'<div class="quick-drawer-panel"><header><strong>快捷飲品｜待補 '+missing+'</strong><span class="quick-drawer-tools"><button data-action="quick-drink-specified">指定餐點</button><em class="quick-scroll-hint">左右滑動 ›</em></span><button data-action="toggle-quick-drawer">×</button></header>'+context+'<div>'+order.filter(d=>d.available!==false).map(d=>drinkChoiceCard(d,'quick-drink',modal?.type==='drink'&&modal.drinkId===d.id,'drawer')).join('')+'</div></div>':'')+'</section>';
}
...
function handle(button,anchorOverride=null){
  ...
  else if(action==='quick-drink-specified'){
    const missing=pendingSummary(store.get().cart).drink;if(!missing){showToast('目前沒有待補飲品');return;}
    modal={type:'completion',dirty:false,draft:{activeGroup:'drink',activeTarget:'',assignments:{}}};render();
  }
