...
function prepareEditButtons(){
  app.querySelectorAll('.cart-actions .edit-button').forEach(button=>{
    const category=categoryName(button.closest('.cart-category'));
    const isCombo=category.includes('飯糰套餐')||category.includes('飯團套餐');
    const label=isCombo?'拆':'改';
    if(button.textContent.trim()!==label)button.textContent=label;
    button.setAttribute('aria-label',isCombo?'修改或拆開套餐':'修改產品');
    button.setAttribute('title',isCombo?'修改或拆開套餐':'修改產品');
  });
}
...
function prepareModalSections(){
  const sections=[
    ...app.querySelectorAll('.specified-link-card .pairing-body>section'),
    ...app.querySelectorAll('.combo-editor-card .combo-role')
  ];
  sections.forEach((section,index)=>{
    const trigger=section.querySelector(':scope>strong')||section.querySelector(':scope>header');
    if(!trigger)return;
    const key=modalSectionKey(section,index);
    section.dataset.foldKey=key;
    section.classList.add('foldable-section');
    trigger.classList.add('fold-trigger');
    trigger.setAttribute('role','button');
    trigger.setAttribute('tabindex','0');
    const collapsed=collapsedModalSections.has(key);
...
function restoreScrollPositions(){
  app.querySelectorAll('.pairing-body,.combo-editor-card .product-settings-body').forEach(node=>{
    const key=scrollKey(node);
    if(key&&scrollMemory.has(key))node.scrollTop=scrollMemory.get(key);
  });
}
...
function positionPopover(card){
  ...
  if(remembered){
    applyPlacement(card,remembered,size);
    return;
  }
