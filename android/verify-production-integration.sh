...
for file in "${required[@]}"; do
  test -s "$file" || { echo "Missing production integration file: $file" >&2; exit 1; }
done
...
grep -Fq 'bundle.update.getStatus' "$SRC/BridgeProtocol.kt"
grep -Fq 'bundle.health-timeout' "$SRC/BridgeProtocol.kt"
grep -Fq 'RuntimeHealthReceiver.cancel(context)' "$SRC/BridgeProtocol.kt"
grep -Fq 'otaUpdate' "$SRC/BridgeProtocol.kt"
grep -Fq 'runtimeHealth' "$SRC/BridgeProtocol.kt"
...
echo 'Production Integration static contract PASS'
