...
const responsive=fs.readFileSync(new URL('../pages/more/responsive.css',import.meta.url),'utf8');
const sharedResponsive=fs.readFileSync(new URL('../shared/responsive-pages.css',import.meta.url),'utf8');
...
const required=[
  ...
  'body[data-page="more"] .more-analysis{',
...
for(const token of required)assert.ok(responsive.includes(token),`missing page-owned responsive More contract: ${token}`);
assert.equal(sharedResponsive.includes('body[data-page="more"]'),false,'shared responsive core must not regain More component ownership');
...
console.log('MORE_RESPONSIVE_PAGE_AUTHORITY_CONTRACT_OK');
