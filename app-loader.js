...
const BUILD='smt-t2s-1280x800-rebuild.19';
...
function applyT2SViewport(){
  ...
  stage.style.width=TARGET_WIDTH+'px';stage.style.height=TARGET_HEIGHT+'px';
  ...
  stage.dataset.viewportWidth=String(size.width);stage.dataset.viewportHeight=String(size.height);stage.dataset.scale=scale.toFixed(4);stage.dataset.fitted='1';
...
function load({force=false}={}){const key=route();if(!force&&key===current)return;current=key;childReady=false;frame.src=routes[key]+'?build='+encodeURIComponent(BUILD);}
frame.addEventListener('error',()=>showLoaderError('子頁載入失敗，資料仍保存在本機。'));
addEventListener('hashchange',()=>load());addEventListener('pageshow',applyT2SViewport);addEventListener('resize',applyT2SViewport,{passive:true});addEventListener('orientationchange',()=>setTimeout(applyT2SViewport,120),{passive:true});
addEventListener('message',event=>{if(event.source!==frame.contentWindow)return;if(event.data?.type==='morefun:page-ready')childReady=true;if(event.data?.type==='morefun:navigate')location.hash='#/'+event.data.route;if(event.data?.type==='morefun:exit-fullscreen'&&document.fullscreenElement)document.exitFullscreen?.();if(event.data?.type==='morefun:set-ui-scale')frame.contentWindow?.postMessage({type:'morefun:ui-scale-disabled',reason:'T2S 使用固定 1280×800 測試框；請使用瀏覽器雙指縮放檢查細節。'},'*');});
applyT2SViewport();load();setTimeout(()=>{if(!childReady)frame.contentWindow?.postMessage({type:'morefun:ping'},'*');},2500);
