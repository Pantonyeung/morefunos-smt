import {createViewportFitter} from './shared/runtime.js?v=16b';
...
function load(){
  ...
  current=key;
  frame.src=routes[key]+'?build=v16b';
}
frame.addEventListener('error',function(){showLoaderError('子頁載入失敗，資料仍保存在本機。');});
addEventListener('hashchange',load);
addEventListener('message',function(event){
  if(event.source!==frame.contentWindow)return;
  if(event.data&&event.data.type==='morefun:navigate')location.hash='#/'+event.data.route;
  if(event.data&&event.data.type==='morefun:page-runtime-error')console.error(event.data);
});
load();
