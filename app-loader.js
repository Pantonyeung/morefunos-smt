...
const BUILD_ID = 'smt-master-v1-20260719';
const LEGACY_RESET_KEY = 'morefun-smt-master-cache-reset-v1';
...
let readyTimer = 0;
let lastGeometry = '';
let lastOrientation = screen.orientation?.type || (innerWidth > innerHeight ? 'landscape' : 'portrait');
...
function armReadyTimeout(route) {
  clearTimeout(readyTimer);
  readyTimer = window.setTimeout(() => showPageError(route), 9000);
}
...
function handleOrientationChange() {
  const nextOrientation = screen.orientation?.type || (innerWidth > innerHeight ? 'landscape' : 'portrait');
