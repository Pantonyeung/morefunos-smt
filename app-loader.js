...
function fitLandscapeOnce(force=false){const size=viewportSize();const orientation=size.width>size.height?'landscape':'portrait';document.documentElement.dataset.orientation=orientation;if(orientation!=='landscape')return;if(!force&&lastOrientation==='landscape'&&stage.dataset.fitted==='1')return;lastOrientation='landscape';clearTimeout(fitTimer);fitTimer=setTimeout(()=>{const stable=viewportSize();const scale=Math.min(stable.width/1920,stable.height/1080);const left=Math.max(0,(stable.width-1920*scale)/2);const top=Math.max(0,(stable.height-1080*scale)/2);stage.style.transform='translate('+left+'px,'+top+'px) scale('+scale+')';stage.dataset.scale=scale.toFixed(6);stage.dataset.fitted='1';},220);}
...
function load(){const key=route();if(key===current)return;current=key;frame.src=routes[key]+'?build=v16g';}
